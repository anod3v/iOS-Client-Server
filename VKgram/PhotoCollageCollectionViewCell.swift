...
class PhotoCollageCollectionViewCell: UICollectionViewCell {
    ...
    func configure(for model: AttachmentPhoto) {
//        url = model
//        guard let path = model?.path else {
//            return postImage.image = UIImage(named: "not found")
//        }
        postImage.loadImageUsingCacheWithURLString(model.photo604!, placeHolder: nil) { (bool) in
    ...
    func configureEmptyCell(for model: AttachmentPhoto, labelText: String) {
//        url = model
//        guard let path = model?.path else {
//            return postImage.image = UIImage(named: "not found")
//        }
